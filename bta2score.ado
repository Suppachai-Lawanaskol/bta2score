...
program define bta2score , rclass
	...
	**Tabulation the rounded score**
	if "`tabulation'"!=""{
		di in green  _column(5) "{hline 40}" _newline(1) _column(5) "Endpoint" _column(30) "`endpoint'"_newline(1) _column(5) "{hline 40}" _newline(1) ///
_column(5) "Predictors" _column(30) "`name'" _newline(1) ///
_column(5) "{hline 40}"
		forvalues x=1/`=scalar(num)'{
			if length("`: word `x' of `: colnames devcoef''")>20{
			di in green _column(5) substr("`: word `x' of `: colnames devcoef''",1,10)+"~"+substr("`: word `x' of `: colnames devcoef''",length("`: word `x' of `: colnames devcoef''")-9,10) in yellow _column(30) roundcoef[1,`x']
			}
			else{
				di in green _column(5) "`: word `x' of `: colnames devcoef''"  in yellow _column(30) roundcoef[1,`x']
			}
		}
		di in green _column(5) "{hline 40}"
	}
	...
	**Detect the variable score name**
	if "`replace'"!=""{
		...
		if !_rc {
			di in green _column(5)"`name' was deleted"
			drop `name'
		}
	}
	else{
		capture confirm variable `name'
		if !_rc {
			di in green _column(5)"`name' already exists"
			drop _`name'*
		}
	}
	...
	if "`cstat'"!=""{
		**Concordant statistic from binary regession, from glm, logit, logistic etc**
		if "`e(cmd)'"=="glm" | "`e(cmd)'"=="regress" | "`e(cmd)'"=="logit" | "`e(cmd)'"=="logistic"{
			...
			display in green _col(5)"Score Harrell's C-statistic" in yellow _column(35) %9.4f `=scalar(cstat)'
		}
		else{
	...
	**D-statistic**
	if "`dstat'"!=""{
		if "`e(cmd2)'"=="stcox"{
			...
			display in green _col(5)"Score Somer's D-statistic" %9.4f in yellow _column(35)`=scalar(dstat)'
	...
	**AIC**
	if "`aic'"!=""{
		if "`e(cmd)'"=="glm" | "`e(cmd)'"=="regress" | "`e(cmd)'"=="logit" | "`e(cmd)'"=="logistic" | "`e(cmd)'"=="ologit" {
			...
			display in green _column(5)"Score AIC" in yellow  %9.2f _column(35) `=scalar(aic)'
	...
	**BIC**
	if "`bic'"!=""{
		if "`e(cmd)'"=="glm" | "`e(cmd)'"=="regress" | "`e(cmd)'"=="logit" | "`e(cmd)'"=="logistic" | "`e(cmd)'"=="ologit" {
			...
			display in green _column(5)"Score BIC" in yellow %9.2f _column(35) `=scalar(bic)'
	...
	**GOF**
	if "`gof'"!=""{
		if "`e(cmd)'"=="glm" | "`e(cmd)'"=="regress" | "`e(cmd)'"=="logit" | "`e(cmd)'"=="logistic"{
			...
			display in green _col(5)"Score Hosmer-Lemeshow GOF" in yellow %9.2f _column(35)`=scalar(gof)'
		}
		else if "`e(cmd)'"=="ologit"{
			if "`e(stepwise)'"=="stepwise"{
				display in green _column(5)"The Final model is under stepwise command prefix"
			}
			else{
			...
			display in green _col(5)"The Final model is not support GOF"
		}
	...
	**R-sqaure**
	if "`rsquare'"!=""{
		if "`e(cmd)'"=="glm"{
			...
			display in green _col(5)"Score R-sqaure" in yellow %9.2f _column(35)`=scalar(rsquare)'
